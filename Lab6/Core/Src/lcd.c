...
void lcd_draw_line_chart(
        uint16_t origin_x, uint16_t origin_y,   // gốc vẽ chart
        uint16_t width, uint16_t height,
        char *title,
        uint16_t color_axis,
        uint16_t color_line,
        uint16_t color_point,
        uint32_t data_x[5],     // X pixel tuyệt đối (tính từ gốc chart)
        uint32_t data_y[5]      // Y pixel tuyệt đối (tính từ gốc chart)
) {
	 uint16_t margin_top = 40;
	    uint16_t margin_left = 10;

	    uint16_t chart_x1 = origin_x + margin_left;
	    uint16_t chart_y1 = origin_y + margin_top;
	    uint16_t chart_x2 = origin_x + width;
	    uint16_t chart_y2 = origin_y + height;

	    // Xoá nền vùng chart
	    lcd_fill(origin_x - 10, origin_y, origin_x + width, origin_y + height + 10, WHITE);

	    // Tiêu đề (center trong toàn chiều rộng màn)
	    lcd_show_string_center(origin_x, origin_y + 10, title, color_axis, 0xFFFF, 16, 0);

	    // Vẽ trục (dựa trên chart_x1, chart_y1, chart_y2)
	    lcd_draw_line(chart_x1, chart_y2, chart_x2, chart_y2, color_axis);  // X-axis
	    lcd_draw_line(chart_x1, chart_y1, chart_x1, chart_y2, color_axis);  // Y-axis

	    // Vẽ đường nối: (chart_x1 + offset)
	    for (int i = 0; i < 4; i++) {
	        uint16_t x1 = chart_x1 + data_x[i];
	        uint16_t y1 = chart_y2 - data_y[i];
	        uint16_t x2 = chart_x1 + data_x[i+1];
	        uint16_t y2 = chart_y2 - data_y[i+1];
	        lcd_draw_line(x1, y1, x2, y2, color_line);
	    }

	    // Vẽ điểm
	    for (int i = 0; i < 5; i++) {
	        uint16_t px = chart_x1 + data_x[i];
	        uint16_t py = chart_y2 - data_y[i];
	        lcd_draw_circle(px, py, color_point, 2, 1);
	    }
}
