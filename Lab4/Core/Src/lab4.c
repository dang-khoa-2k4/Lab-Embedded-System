...
uint16_t button_hold[16];
static uint16_t button_hold_counter[16] = {0};
...
void displayTime()
{
	...
	lcd_ShowIntNum(150, 130, 2000 + ds3231_year, 2, YELLOW, BLACK, 24);
}
...
{   
    ...
    if (isButtonUp())
    {   
        if (button_count[3] >= PRESS_DURATION)
        {
            button_hold_counter[3]++;
            if (button_hold_counter[3] % AUTO_INCREMENT_PERIOD == 0)
            {
                time_setting_temp[cur_setting] = 
                    (time_setting_temp[cur_setting] + 1) 
                    % (max_values[cur_setting] + 1);
            }
        }
        else
        {
            button_hold_counter[3] = 0;
            if (button_count[3] == 1)
                time_setting_temp[cur_setting] = 
...
void alarmSetting()
{
    ...
    if (firstEntry)
    {
        alarm_time_temp[HOUR - 4] = time_setting[HOUR];
        alarm_time_temp[MIN - 4] = time_setting[MIN];
        alarm_time_temp[SEC - 4] = time_setting[SEC];
        firstEntry = 0;
    }
