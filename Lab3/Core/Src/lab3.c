...
static void write_lcd(char *str, uint16_t textColor, uint16_t bgColor) {
    lcd_Fill(0, 20, 240, 54, WHITE);
    lcd_StrCenter(0, 20, str, textColor, bgColor, 24, 0);
}
...
static void handle_modification_buttons(int8_t *config_time) {
    ...
    if (button_count[BTN_DEC] == 1) {
        temp_mod_time--;
        if (temp_mod_time < MIN_TIME) 
            temp_mod_time = MAX_TIME;
    }
...
void write_traffic_light_1(void) {
    ...
    lcd_DrawCircle(60, 120, BLACK, 25, 1);
    lcd_DrawCircle(120, 120, BLACK, 25, 1);
    lcd_DrawCircle(180, 120, BLACK, 25, 1);
}

void write_traffic_light_2(void) {
    lcd_ShowStr(10, 160, "Light 2", BLACK, WHITE, 24, 0);
    lcd_ShowStr(175, 160, "00", RED, BLACK, 24, 0);
    lcd_DrawCircle(60, 220, BLACK, 25, 1);
    lcd_DrawCircle(120, 220, BLACK, 25, 1);
    lcd_DrawCircle(180, 220, BLACK, 25, 1);
}
...
void fsm_traffic(void) {
    switch (state) {
        ...
        case BEGIN:
            ...
            write_lcd("MODE : NORMAL", GBLUE, BLACK);
        ...
        case MOD_RED:
            ...
            if (button_count[BTN_MODE] == 1) {
                ...
                blink_counter = 0;
                write_lcd("MODE : MODIFICATION", WHITE, GREEN);
            }
            break;
        ...
        case MOD_GREEN:
            ...
            if (button_count[BTN_MODE] == 1) {
                ...
                blink_counter = 0;
                write_lcd("MODE : MODIFICATION", WHITE, YELLOW);
            }
            break;
